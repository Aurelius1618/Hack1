...
class ScreenerAgent:
    ...
    def calculate_altman_z_score(self, company_data: Dict) -> float:
        ...
        try:
            ...
            working_capital = self.data_processor._safe_get(company_data, 'working_capital', 0)
            total_assets = self.data_processor._safe_get(company_data, 'total_assets', 1)  # Avoid division by zero
            retained_earnings = self.data_processor._safe_get(company_data, 'retained_earnings', 0)
            ebit = self.data_processor._safe_get(company_data, 'ebit', 0)
            market_value_equity = self.data_processor._safe_get(company_data, 'market_value_equity', 0)
            total_liabilities = self.data_processor._safe_get(company_data, 'total_liabilities', 1)  # Avoid division by zero
            sales = self.data_processor._safe_get(company_data, 'sales', 0)
            
            # Calculate ratios
            t1 = working_capital / total_assets
            t2 = retained_earnings / total_assets
            t3 = ebit / total_assets
            t4 = market_value_equity / total_liabilities
            ...
            logger.error(f"Error calculating Altman Z-Score: {str(e)}")
            return 0.0
    ...
    def generate_company_comparison(self, company1: str, company2: str) -> Dict[str, Any]:
        ...
        """
        if self.data_processor.company_df is None:
            return {
                "status": "error",
                "message": "Company data not loaded",
                "data": None
            }
        
        # Find companies in the database
        company1_data = self.data_processor.company_df[self.data_processor.company_df['company_name'] == company1]
        company2_data = self.data_processor.company_df[self.data_processor.company_df['company_name'] == company2]
        
        if company1_data.empty:
            return {
                "status": "error",
                "message": f"Company not found: {company1}",
                "data": None
            }
        ...
        if company2_data.empty:
            return {
                "status": "error",
                "message": f"Company not found: {company2}",
                "data": None
            }
        
        # Extract financial metrics
        metrics = ["ROE", "Debt/Equity", "Interest Coverage", "Current Ratio", "Quick Ratio", "EBITDA Margin"]
        company1_values = []
        company2_values = []
        
        # Get calculated ratios
        try:
            company1_ratios = json.loads(company1_data['calculated_ratios'].iloc[0]) if not pd.isna(company1_data['calculated_ratios'].iloc[0]) else {}
            company2_ratios = json.loads(company2_data['calculated_ratios'].iloc[0]) if not pd.isna(company2_data['calculated_ratios'].iloc[0]) else {}
            
            # ROE
            company1_values.append(company1_ratios.get('roe', 0) * 100)  # Convert to percentage
            company2_values.append(company2_ratios.get('roe', 0) * 100)  # Convert to percentage
            
            # Debt/Equity
            company1_values.append(company1_ratios.get('debt_to_equity', 0))
            company2_values.append(company2_ratios.get('debt_to_equity', 0))
            
            # Interest Coverage
            company1_values.append(company1_ratios.get('interest_coverage_ratio', 0))
            company2_values.append(company2_ratios.get('interest_coverage_ratio', 0))
            
            # Current Ratio
            company1_values.append(company1_ratios.get('current_ratio', 0))
            company2_values.append(company2_ratios.get('current_ratio', 0))
            
            # Quick Ratio
            company1_values.append(company1_ratios.get('quick_ratio', 0))
            company2_values.append(company2_ratios.get('quick_ratio', 0))
            
            # EBITDA Margin
            company1_values.append(company1_ratios.get('ebitda_margin', 0) * 100)  # Convert to percentage
            company2_values.append(company2_ratios.get('ebitda_margin', 0) * 100)  # Convert to percentage
        ...
        comparison = {
            "metrics": metrics,
            company1: company1_values,
            company2: company2_values
        }
        
        return {
            "status": "success",
            "message": f"Comparison generated for {company1} and {company2}",
            "data": comparison
        }
