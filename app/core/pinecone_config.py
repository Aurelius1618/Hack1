...
def initialize_pinecone():
    ...
    try:
        ...
        # Define indexes to create with optimized configuration
        indexes = {
            "bond_metadata": {
                "dimension": 768,
                "metric": "dotproduct",
...
def hybrid_search(query, namespace="BOND_METADATA", top_k=5, alpha=0.7):
    ...
    Args:
        ...
        namespace (str): The namespace to search in (use the key from NAMESPACES)
    ...
    namespace_config = NAMESPACES.get(namespace, NAMESPACES["BOND_METADATA"])
    namespace_path = namespace_config["namespace"]
    ...
    # Get the index
    index_name = namespace_path.split('/')[0]
    ...
    try:
        results = index.query(
            **hybrid_query.to_dict(),
            top_k=top_k,
            namespace=namespace_path.split('/')[1] if '/' in namespace_path else None,
...
# Define the namespaces for our different data types with content type information
NAMESPACES = {
    "BOND_METADATA": {
