...
def route_query(state: AgentState) -> AgentState:
    ...
    Uses Lamini finetuned model for classification
    """
    query = state["query"]
    ...
    if re.search(ebit_pattern, query, re.IGNORECASE):
        ...
        # Use the Lamini classification for other queries
        try:
            routing_data = classify_query(query)
            logger.info(f"Classified query using Lamini: {routing_data}")
        except Exception as e:
            logger.error(f"Error classifying query with Lamini: {str(e)}")
            # Fallback to keyword-based classification
            routing_data = fallback_classify_query(query)
            logger.info(f"Classified query using fallback method: {routing_data}")
    ...
    # Add contrastive CoT reasoning if available
    try:
        reasoning = actor_critic_flow(query)
        if reasoning and "reasoning" in reasoning:
            state["reasoning_chain"].append(f"Reasoning: {reasoning['reasoning']}")
    except Exception as e:
